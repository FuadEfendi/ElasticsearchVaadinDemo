...
public class BookExample extends CaptionedExampleItem {
    ...
    public List<SourceFragment> getSourceFragments() {
        ...
        MyUI.getLogger().warn(Arrays.toString(sourceFragments.toArray()));
    ...
    private List<SourceFragment> readSourcesFromClass(String className,
        ...
        */
        ...
        if (ins == null) {
            ...
            MyUI.getLogger().info("Trying to open " + srcname + " for reading.");
            ins = getClass().getResourceAsStream(srcname);
            if (ins == null)
                MyUI.getLogger().warn("Couldn't even open " + srcname + " for reading.");
        }

        if (ins != null)
            return readSourceFromStream(ins, exampleId, srcname);
        else
            MyUI.getLogger().warn("Error: unable to open " + srcname + " for reading.");
    ...
    private List<SourceFragment> readSourceFromStream(InputStream ins,
        ...
        try {
            ...
            MyUI.getLogger().info("Could not read example source code because: " + e.getMessage());
            return null;
        }
